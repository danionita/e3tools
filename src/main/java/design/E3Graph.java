...
public class E3Graph extends mxGraph implements Serializable {
    ...
    public final ArrayList<String> valueObjects = new ArrayList<>(
            Arrays.asList("MONEY", "MONEY-SECURED", "SERVICE", "PRODUCT")
    );
    ...
    /**
     * Indicates whether or not the graph POSSIBLY (might not!) contains any
     * changes and should be saved before exiting. This is set to true on any
     * change (either redo's, undo's, or other kind of changes). Saving it with
     * the GraphIO class sets it to false again.
     */
    ...
    /**
     * Constructs a copy of graph original with changes indicated in delta.
     * After applying the changes the valuations from all value transfers are
     * also propagated to value ports.
     */
...
//				System.out.println("Colluding: " + id);
    ...
    /**
     * Returns true if given cell is a fitting drop target for cells. Hierarchy:
     * - Actor can contain market segments and actors, or ONLY value activities
     * - Market segment can contain value activities - Value activities cannot
     * contain anything As of 2016-8-16. This function is only called if a node
     * is dropped inside another node. The top level filtering (that makes sure
    ...
    /**
     * Tells mxGraph that when the cell is a ValueInterface it should be
     * "constrained". The constrainment behaviour is implemented in
    ...
    /**
     * If the cell is a valueinterface, returns the closest side that is
     * parallel to the value interface
     *
     */
    @Override
     ...
     *
     * Applies constrainment when the cell is out of bounds of the containment
     * area. This seems to be different from the default implemented behaviour,
     * altough this seems more appropriate.
     *
     * The geometry is not cloned because the function that is overridden does
     * not either, and we should not break the contract.
     */
    @Override
    ...
    public static void straightenValueInterface(mxGraph graph, mxICell vi) {
        if (!(Utils.base(graph, vi) instanceof ValueInterface)) {
            return;
        }
        ...
        try {
            ...
            if (parentGm != null) {
                ...
                if (min == top || min == bottom) {
                    ...
                    if (min == top) {
                        viInfo.side = Side.TOP;
                    }
                    if (min == bottom) {
                        viInfo.side = Side.BOTTOM;
                    }
                } else {
                    viGm.setWidth(20);
                    viGm.setHeight(30);
    ...
    /**
     * Adds a valueport to valueinterface cell vi. This is a static function
     * because it seems separate functionality from the actual graph class (non
     * e3fraud-graphs can also use this functionality now. It could be
     * refactored just fine though.
     *
     * @param graph
     * @param vi
     ...
     * toString() methods to return a nice name for a label. Everything else
     * should use the default behaviour.
     */
    @Override
    ...
    /**
     * Is called when the user doubleclicks an (editable) node, types a new
    ...
    /**
     * Connects the first outgoing valueport in start with the first incoming
     * value port on end.
     *
    ...
    /**
     * Connects pairs of signaldots, or pairs of start signal/end signal/value
     * interfaces So the allowed combinations are: (SignalDot/LogicDot) ->
     * (SignalDot/LogicDot) (start signal/end signal/value interface) -> (start
     * signal/end signal/value interface)
     *
     * @param start
     * @param end
     * @return
...
//			Utils.updateValueExchangeValueObjectLabel(this, ve);
     ...
     * @param unit Whether or not to hook the signal to the unit dot or a normal
     * logic dot
    ...
    public Object connectLogicToLogic(Object l, Object r, boolean unitL, boolean unitR) {
        ...
        for (Object dotL : logicDotsL) {
            if (getModel().getEdgeCount(dotL) > 0) {
                continue;
            }
            for (Object dotR : logicDotsR) {
                if (getModel().getEdgeCount(dotR) > 0) {
                    continue;
                }
    ...
    /**
     * Turns a value model into a fraud model, or simply copies the model if it
     * is already a fraud model. If it is a value model also prepends "Fraud
     * model of " to the title. Lossless conversion.
     *
     * @return
     */
    ...
    /**
     * Converts the graph to a value graph if it is a fraud graph, and simply
     * copies it otherwise. Also prepends "Value model of " if it is a fraud
     * model to the title. Lossy conversion (colluded actors are turned off,
     * hidden transactions are deleted, non-occurring transactions are turned
     * off).
     *
     * @return
     */
    ...
    /**
     * Returns the container (actor/market segment/value activity) of a value
     * port or signal dot.
     *
     * @param vp
     * @return
     */
    public Object getContainerOfChild(Object vp) {
    ...
    public boolean isValidConnection(Object source, Object target) {
        ...
        if (sourceVal instanceof ValuePort && targetVal instanceof ValuePort) {
            ...
            Object sourceContainer = getContainerOfChild(source);
            Object targetContainer = getContainerOfChild(target);
    ...
    /**
     * If the returned string is non-null and has length > 0 then it means the
     * cell was not valid.
     */
    @Override
    ...
    /**
     * Returns true if the graph contains at least two actors actors false if
     * not.
     */
    ...
    public long countE3ObjectsOfType(Class<?> e3Class) {
        return Utils.getAllCells(this).stream()
                ...
                .filter(cellValue -> e3Class.isInstance(cellValue))
                .count();
    }
    ...
    /**
     * Forces propagation of valuations of all value exchanges in the graph to
     * all value ports.
     */
